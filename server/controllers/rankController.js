...
export const getkeyword = async(req,res) => {
     try {
        const tracking = await KeywordTracking.findOne({_id: req.params.id});
...
try {
        const tracking = await KeywordTracking.findOneAndDelete({_id: req.params.id});
        if(!tracking) return res.status(404).json({success: false , message: "Keyword tracking not found"});
...
export const toggleTracking = async(req,res) => {
    try {
        ...
        console.error("Toggle keywords error:" , error.message);
        res.status(500).json({success: false , message: "server error"});
    }
}